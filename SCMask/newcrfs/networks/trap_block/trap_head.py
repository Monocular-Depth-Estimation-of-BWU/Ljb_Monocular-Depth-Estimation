...
def trapped_inter(x):
    """
    对输入张量进行特定变换的函数,不变通道数变大小
    
    Args:
        x (torch.Tensor): 输入张量，形状为 (B, C, H, W)，分别代表批量大小、通道数、高度和宽度
    
    Returns:
        torch.Tensor: 输出张量，形状为 (B, C, H * 2, W * 2)，对输入张量进行一系列变换后得到的结果
    ...
    mean_value = x.mean()
    min_value = mean_value / 3
    max_value = (mean_value + x.max()) / 1.5
    condition = (x >= min_value) & (x <= max_value)

    mask3 = torch.round(torch.abs(torch.cos(x))) 
    #mask4 = torch.round(torch.sin(x)**2)
    drop = torch.rand(1).item() #随机生成0或1
    mask4 = torch.randn((B, C, H, W)) > drop #使用类似drop的随机遮罩即可实现像素筛选
    #mask4 = torch.randint(0, 2, (B, C, H, W) ) #上下同理，上者生成0-1的正太分布，下者0，1整数
    mask4 = mask4.to('cuda:0')
    

    x1 = mask1 * x
    #x2 = mask2 * x
    x2 =torch.where(condition,torch.zeros_like(x), x)
    x3 = mask3 * x
    x4 = mask4 * x
